...
//  Future Additions
//  Rain sensor to restrict watering to only when it isn't raining or hasn't just rained
...
// 8: Display temperature history over the past 24 hours, 0 is th most current temperature stored
...
int menu[] = { 6, 1 };   //  Determines display and menu navigation
...
void rotary_encoder() {  // Read change in rotary encoder or push-button
  ...
  if (rotaryLast != rotary && menu[0] != 6) {  // If rotary changes, update menu
  ...
  if (digitalRead(buttonPin) == LOW) {  // Buton is pressed
    ...
    if (menu[0] == 1) {
	  if (menu[1] == 8) {  // Select to view temperature history
	    tempHistS = 0;
		menu[0] = 12;
      } 
	  else if (menu[1] == 7) {  // Reset overrides
        schedule.pumpOR = 3;
        schedule.valveOR = 3;
        EEPROM_writeAnything(0, schedule);
		rotary = 1;
      ...
      else if (menu[1] == 6) {  // Select schedule On/Off, go to default dispay
		menu[0] = 11;
      } 
	  else if (menu[1] == 5) {  // Select Valve On/Off
	    schedule.valveOR = 0;
        EEPROM_writeAnything(0, schedule);
	    menu[0] = 10;
      }
      else if (menu[1] == 4) {  // Select Pump On/Off
	    schedule.pumpOR = 0;
        EEPROM_writeAnything(0, schedule);
	    menu[0] = 9;
      }
	  else if (menu[1] == 3) {  // Select Change schedule
		menu[0] = 3;
      }
	  else if (menu[1] == 2) {  // Select Display schedule 
	    menu[0] = 2;
      }
      else {  // Select Resume
	    blink_timer[0] = lWaitMillis;
    ...
    else if (menu[0] == 2) {  // On display schedule
	  rotary = 1;
	  blink_timer[0] = lWaitMillis;
	  menu[0] = 6;
    }
    else if (menu[0] == 3) {  // Change schedule: 1. Select how many schedules to water
      counttimes = 1;
	  menu[0] = 4;
    }
    else if (menu[0] == 4) {  // Change schedule: 2. Select time
	...
	else if (menu[0] == 5) {  // Change schedule: 3. Select how long to run pump for
	  if (counttimes == schedule.numofschedules) {
	  	counttimes = 1;
	    menu[0] = 7;
	  }
	...
	else if (menu[0] == 7) {  // Change schedule: 4. Select how many days between running pump
	  if (counttimes == schedule.numofschedules) {
	    counttimes = 1;
	    menu[0] = 8;
		if (schedule.days1TOwater == 1) {
		  if ((int)hour < schedule.time1hour || ((int)hour == schedule.time1hour && (int)minute <= schedule.time1minute)) schedule.nextday1TOwater = dayOfWeek;
		...
		if (schedule.days2TOwater == 1) {
		  if ((int)hour < schedule.time2hour || ((int)hour == schedule.time2hour && (int)minute <= schedule.time2minute)) schedule.nextday2TOwater = dayOfWeek;
		...
		if (schedule.days3TOwater == 1) {
		  if ((int)hour < schedule.time3hour || ((int)hour == schedule.time3hour && (int)minute <= schedule.time3minute)) schedule.  nextday3TOwater = dayOfWeek;
	...
	else if (menu[0] == 8) {  // Change schedule: 5. Select the starting day of schedule
	  if (counttimes == schedule.numofschedules) {
	    schedule.onoff = 1;
		blink_timer[0] = lWaitMillis;
	    menu[0] = 6;
    ...
    else if (menu[0] == 6) {  // On current time
	...
	else if (menu[0] == 9 || menu[0] == 10 || menu[0] == 11 || menu[0] == 12) {  // On change pump, valve, schedule
	  rotary = 1;
	  blink_timer[0] = lWaitMillis;
	  menu[0] = 6;
	}
...
void seg_display() { // Update the 7-segment display
  ...
  case 5:  // Change schedule: 3. How many seconds to run pump
    ...
    break;
  case 7:  // Change schedule: 4. How many days between running particular schedule
	if (counttimes == 1) {
    ...
    break;
  case 8:  // Change schedule: 5. What day of the week to start schedule
    if (counttimes == 1) {
    ...
    break;
  case 6:  // Display current time and temperature
    if (schedule.emergency != 0) {
	  if (schedule.emergency == 1) {
	    mySerialPort.print("-E-1"); // Emergency 1 shutdown
	  }
	  else if (schedule.emergency == 2) {
	    mySerialPort.print("-E-2"); // Emergency 2 shutdown
	  }
	  else if (schedule.emergency == 3) {
	    mySerialPort.print("-E-3"); // Emergency 3 shutdown
	  }
        }
	else {
	  if (lWaitMillis - blink_timer[0] < 5000) { // if seconds is even
	    ...
	    if (lWaitMillis - blink_timer[0] > 9000) {
	      blink_timer[0] = lWaitMillis;
	      blink_timer[1] = 1;
	    }
      }
...
void relay_control() { // Turn relays On/Off
  EEPROM_readAnything(0, schedule);
  if (schedule.onoff && tempOK && schedule.emergency == 0) {
    if (schedule.nextday1TOwater == dayOfWeek && (int)hour == schedule.time1hour && (int)minute == schedule.time1minute && second < 3 && !schedule.pumptrigger && schedule.pumpOR != 0) {
      relay_timer = lWaitMillis;
	  schedule.pumptrigger = true;
	  timer = 1;
	  EEPROM_writeAnything(0, schedule);
    }
    else if (schedule.nextday2TOwater == dayOfWeek && (int)hour == schedule.time2hour && (int)minute == schedule.time2minute && second < 3 && !schedule.pumptrigger && schedule.pumpOR != 0 && schedule.numofschedules > 1) {
	  relay_timer = lWaitMillis;
      schedule.pumptrigger = true;
	  timer = 2;
	  EEPROM_writeAnything(0, schedule);
    }
    else if (schedule.nextday3TOwater == dayOfWeek && (int)hour == schedule.time3hour && (int)minute == schedule.time3minute && second < 3 && !schedule.pumptrigger && schedule.pumpOR != 0 && schedule.numofschedules == 3) {
	  relay_timer = lWaitMillis;
	  schedule.pumptrigger = true;
	  timer = 3;
	  EEPROM_writeAnything(0, schedule);
  ...
  if (schedule.pumptrigger) {
    if (timer == 1 && (lWaitMillis - relay_timer)/1000 >= schedule.seconds1TOwater) {
	  pressure_timer = 0;
	  relay_timer = lWaitMillis;
    ...
    if (timer == 2 && (lWaitMillis - relay_timer)/1000 >= schedule.seconds2TOwater) {
	  pressure_timer = 0;
	  relay_timer = lWaitMillis;
    ...
    if (timer == 3 && (lWaitMillis - relay_timer)/1000 >= schedule.seconds3TOwater) {
	  pressure_timer = 0;
	  relay_timer = lWaitMillis;
  ...
  if (schedule.emergency == 0 && pressure_timer == 0 && (schedule.pumpOR == 1 || (schedule.pumptrigger && schedule.pumpOR == 3))) {
    digitalWrite(pump_RelayPin, HIGH);
    pressure_timer = lWaitMillis;
  }
  else if (schedule.pumpOR == 0 || (!schedule.pumptrigger && schedule.pumpOR == 3)) {
	digitalWrite(pump_RelayPin, LOW);
	pressure_timer = 0;
	relay_timer = lWaitMillis;
	schedule.pumptrigger = false;
  ...
  // After rain sensor is installed, add the next line (or alteration) to the above if conditional statements
  // && analogRead(RainsensorPin) != HIGH
}
...
void tempPredict() { //  Predict future temperature and check if it's within an acceptable range for plant tolerance
  ...
  EEPROM_readAnything(0, schedule);
  if (tempF < 60 && tempF < (schedule.tempHist[1]+schedule.tempHist[2]+schedule.tempHist[3])/3) {
    if (tempF < 37 || ((tempF < schedule.tempHist[2] < schedule.tempHist[4]) && (tempF - (((schedule.tempHist[3] - schedule.tempHist[2])+(schedule.tempHist[2] - schedule.tempHist[1]))/2)*4 < 35)) || ((tempF < avgTemp(3)) && (tempF - (2*(avgTemp(3) - tempF)) < 35))) tempOK = false;
    else tempOK = true;
  }
  else tempOK = true;
  EEPROM_writeAnything(0, schedule);
...
void status() { // Send debug messages to serial terminal
  ...
  Serial.println();
  Serial.print(" lw-pre:");
  Serial.print(lWaitMillis - pressure_timer);
  Serial.println();
}
...
void loop() {
  ...
  if ((long)(millis() - lWaitMillis) >= 0) {     //  Perform once per second to improve responsiveness
    ...
    if ((int)minute == 0 && (int)second == 0) {  //  Every hour: get temperature & add temperature to an array (24 hours)
      tempPredict();
      EEPROM_readAnything(0, schedule);
  ...
  //    else dayOfWeek = 1;}}
    ...
    if ((digitalRead(floatswitchPin) == LOW || lWaitMillis - valve_timer < valvesec*1000 || schedule.valveOR == 1) && schedule.valveOR != 0 && schedule.emergency == 0) {
    ...
    if (!schedule.pumptrigger && lWaitMillis - relay_timer > 180000 && digitalRead(floatswitchPin) == LOW) {
  ...
  if (digitalRead(pump_RelayPin) == HIGH && (analogRead(pressureswitchPin)+analogRead(pressureswitchPin))/2 < 840 && lWaitMillis - pressure_timer > 500) {
    EEPROM_readAnything(0, schedule);
